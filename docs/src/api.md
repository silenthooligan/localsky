...
| `/api/v1/wizard/scan_zones` | POST | `{ "controller": <ControllerEntry> }`; zone discovery for controllers that support it. Returns `{ "zones": [ { "station_id", "name" } ] }`. The setup wizard imports results as zone entries; the controller editor merges them into the kind's zone map (`zone_uuid_map` / `zone_relay_map` / `zone_station_map`). A redacted secret in the posted entry is restored from the stored config by entry id (`400 unmatched_redacted_secret` when no stored value matches). `502` if unreachable, `422` if the kind cannot enumerate zones |
