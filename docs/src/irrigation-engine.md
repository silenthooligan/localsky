...
When application rate exceeds the modeled infiltration rate, LocalSky can divide watering into cycles with soak gaps. Soil texture, slope, sprinkler rate, and configured soak settings affect the split. The schedule includes those gaps, so valve-open minutes and total elapsed time differ.

A longer allowed session does not remove cycle and soak. Check actual runoff and coverage when tuning these inputs.
