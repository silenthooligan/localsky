...
pub struct ConfigApiState {
    ...
    /// Tuning report generation, for the apply endpoint's stale check;
    /// None without a history database (apply answers 503).
...
async fn apply_runtime_config_if_live(
    runtime: &Option<RuntimeHandles>,
    prev: Option<&Config>,
    new_cfg: &Config,
) -> crate::runtime::ConfigApplyOutcome {
    match runtime {
        Some(h) => crate::runtime::apply_runtime_config(h, prev, new_cfg).await,
        ...
        .filter_map(|kind| {
            ...
            let status =
                crate::api::health::compute_source_status(crate::api::health::SourceStatusInputs {
                    ...
                    reachable: configured.and_then(|entry| {
                        source_reachable
                            .as_ref()
...
async fn put_raw_toml(
    ...
    match store.save(&parsed).await {
        Ok(_) => {
            ...
            let outcome = apply_runtime_config_if_live(&runtime, prev_cfg.as_ref(), &parsed).await;
            Json(serde_json::json!({
                "ok": true,
                "validation": report,
...
/// Every secret string the redaction would replace, so a text that must
/// not carry them (the diagnostics bundle's log lines) can be scrubbed by
/// value. Derived from `redact_secrets` itself: the key list lives in one
/// place.
pub(crate) fn secret_values(v: &serde_json::Value) -> Vec<String> {
    fn leaves(v: &serde_json::Value, out: &mut Vec<String>) {
        match v {
            serde_json::Value::String(s) => out.push(s.clone()),
            serde_json::Value::Array(a) => a.iter().for_each(|x| leaves(x, out)),
            serde_json::Value::Object(o) => o.values().for_each(|x| leaves(x, out)),
            _ => {}
        }
    }
    ...
    leaves(v, &mut before);
    leaves(&redacted, &mut after);
    before
        .into_iter()
        .zip(after)
        .filter(|(b, a)| a == SECRET_REDACTED_SENTINEL && b != a && b.len() >= 4)
        .map(|(b, _)| b)
        .collect()
}
...
async fn put_config(
    ...
    match store.save(&cfg).await {
        Ok(v) => {
            ...
            let outcome = apply_runtime_config_if_live(&runtime, prev_cfg.as_ref(), &cfg).await;
            Json(serde_json::json!({
                "saved": v,
                "validation": report,
                "restart_required": outcome.restart_required,
...
async fn post_zones_apply(
    ...
    match store.save(&cfg).await {
        Ok(saved) => {
            let outcome = apply_runtime_config_if_live(&runtime, Some(&prev_cfg), &cfg).await;
            tracing::info!(
                zone = %zone_key,
                field = %rec.field,
                old = %old_value,
...
async fn post_rollback(
    ...
    match store.rollback(ts).await {
        Ok(cfg) => {
            // The ledger (migration record, seeded ids) is a separate file and
            // rides across a rollback untouched.
...
mod tests {
    ...
    // the ledger is empty for all seven, and put every record
    ...
    fn marker(entity: &str) -> crate::model::HaAdoptedHelper {
    ...
    /// A minimally valid config beside a ledger with records in it.
    ...
    fn ledger_intact(store: &FileConfigStore) {
        let l = store.ledger();
        assert_eq!(l.ha_adoption.len(), crate::ha_adopt::ENTITIES.len());
        for id in crate::ha_adopt::ENTITIES {
            assert!(l.ha_adoption.iter().any(|h| h.entity == id), "{id} dropped");
        }
        assert!(l.seeded_source_ids.contains(&"nws".to_string()));
    }

    /// No whole-config write can drop the ledger: it is not in the document.
    #[tokio::test]
    async fn a_settings_save_cannot_touch_the_ledger() {
        let (store, cfg) = adopted_store("put-json").await;
        let mut body = serde_json::to_value(&cfg).unwrap();
        // A client that thinks it knows the shape and sends records anyway.
        body.as_object_mut()
            .unwrap()
            .insert("ha_adoption".into(), serde_json::json!([]));
        body.as_object_mut()
            .unwrap()
            .insert("seeded_source_ids".into(), serde_json::json!(["bogus"]));
        let resp = put_config(
        ...
        assert!(
            !text.contains("ha_adoption") && !text.contains("bogus"),
            "{text}"
        );
    }

    #[tokio::test]
    async fn a_raw_toml_save_cannot_touch_the_ledger() {
    ...
    /// A rollback restores values; the ledger is not a value and stays.
    #[tokio::test]
    async fn a_rollback_keeps_the_ledger() {
        let (store, before) = adopted_store("rollback").await;
        ...
        let snaps = crate::ports::config_store::ConfigStore::list_snapshots(&*store)
            .await
            .unwrap();
        let ts = snaps.first().expect("a snapshot exists").version;
        ...
        let restored = crate::ports::config_store::ConfigStore::load(&*store)
            ...
            .unwrap();
        assert_eq!(
            restored.engine.skip_rules.max_wind_mph,
            before.engine.skip_rules.max_wind_mph
        );
        ledger_intact(&store);
    }
    ...
    fn redact_toml_str_sanitizes_a_real_config_file() {
        ...
        cfg.notifications.ntfy = Some(NtfyConfig {
            ...
            topic: "lawn".into(),
            auth_token: Some("ntfy_token_secret".into()),
        });
        ...
        assert!(
            !redacted.contains("ntfy_token_secret"),
            "ntfy token leaked in backup TOML"
        );
        assert!(
    ...
    /// is inert there). Mirrors the boot-time construction in boot::api.
    ...
    async fn catalog_recently_observed_mrms_with_stale_reachability_is_not_offline() {
        ...
        let resp = get_source_catalog(State(state.clone()))
            .await
            .into_response();
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
        ...
        reachable.report("mrms_main", false, now);
        let resp = get_source_catalog(State(state)).await.into_response();
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        let failed: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        let mrms = failed["cloud_sources"]
            .as_array()
            .unwrap()
            .iter()
            .find(|e| e["kind"] == "noaa_mrms")
        ...
        assert_eq!(
            mrms["status"], "offline",
            "the catalog must retain the adapter's failure even with recent data"
        );
    }
