...
/// mounted) persists each field as a sensor_history row. Reachability
/// events are logged only; freshness is defined by observations.
pub fn spawn(
...
mod tests {
    ...
    async fn recorder_persists_observation_and_last_seen() {
        ...
        let ls = SourceLastSeen::default();
        spawn(tx.clone(), Some(store.clone()), ls.clone());
