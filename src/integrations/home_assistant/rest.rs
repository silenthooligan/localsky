...
impl HaClient {
    pub fn from_env() -> Result<Self> {
        ...
        // The shared client: an 8 s timeout (HA is on the LAN and /api/states
        // is one round trip) and the derived per-install User-Agent. HA has
        // no UA policy and this is not a keyless authority, so the derived
        // identity replaces the old fixed "localsky/irrigation" string.
        // `net::client` cannot fail; `Result` stays for the env lookups.
        let client = crate::net::client(std::time::Duration::from_secs(8));
        Ok(Self {
            client,
            base_url,
            token,
