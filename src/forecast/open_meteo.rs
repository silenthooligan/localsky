...
pub fn spawn_forecast_refresher(
    ...
    tokio::spawn(async move {
        ...
        // A separate connect budget so a dead host (like the 2026-07
        ...
        loop {
            ...
            let sleep_for = match crate::metrics::observe_fetch(
                &source_id,
                refresh_once(&client, lat, lon, &model, endpoint.as_deref(), past_days),
            )
            .await
            {
                Ok((snap, current)) => {
...
/// `None` = unlocated install: the caller skips the upstream fetch
/// entirely rather than impersonating a default city. (0, 0) means
/// "location never set" and falls through. The environment is not
/// consulted: an env-described install had its config written at boot.
...
struct RawDaily {
    ...
    /// Daily total shortwave radiation, MJ/m2.
    ///
    /// The input FAO-56 Penman-Monteith needs and never had. Without it
    /// the method-selection ladder could not reach PM at all, so it
    /// always fell through to Hargreaves-Samani, which estimates
    /// radiation from the temperature range alone. Requesting it makes
    /// the better method reachable on a cloud-only install.
    #[serde(default)]
...
impl RawHourly {
    fn rain_index_for_start(&self, index: usize) -> Option<usize> {
        let end = self.time.get(index)?.checked_add(3600)?;
        let next = index.checked_add(1)?;
        (self.time.get(next).copied() == Some(end)).then_some(next)
    }
}
    ...
    fn into_snapshot(self) -> ForecastSnapshot {
        ...
        let hourly: Vec<HourlyEntry> = (0..self.hourly.time.len())
            .map(|i| HourlyEntry {
                ...
                // Normalize them to the canonical row's [T,T+1h) interval.
                precip_in: self
                    .hourly
                    .rain_index_for_start(i)
                    .and_then(|j| pick(&self.hourly.precipitation, j))
                ...
                precip_probability: self
                    .hourly
                    .rain_index_for_start(i)
                    .and_then(|j| pick(&self.hourly.precipitation_probability, j)),
                ...
                et0_in: self
                    .hourly
                    .rain_index_for_start(i)
                    .and_then(|j| pick(&self.hourly.et0_fao_evapotranspiration, j))
                    .unwrap_or(0.0),
                et0_reported: self
                    .hourly
                    .rain_index_for_start(i)
        ...
        let all_daily: Vec<DailyEntry> = (0..self.daily.time.len())
            .map(|i| DailyEntry {
                // Open-Meteo stamps 00:00 local under timezone=auto.
                ...
                et0_reported: pick(&self.daily.et0_fao_evapotranspiration, i)
                    .is_some_and(|v| v.is_finite() && v >= 0.0),
                solar_rad_mj_m2_day: pick(&self.daily.shortwave_radiation_sum, i).unwrap_or(0.0),
                wind_mean_2m_ms: None,
        ...
        snap.backfill_daily_et0_means(crate::timeutil::deployment_calendar(), 10.0);
        snap
    }
}
...
///
/// CROSS-AGENT CONTRACT: runtime.rs source_field_names ~:417 returns this
...
impl Raw {
    ...
    fn current_fields(&self) -> Option<(Vec<(WeatherField, f64)>, i64)> {
        ...
        if let Some(pr) = cur.precipitation {
            ...
            // FILL, so it can never read as live station rain (the
...
mod tests {
    ...
    fn default_model_url_is_stable_and_includes_current_block() {
        ...
        let expected = concat!(
            ...
            "apparent_temperature_max,cape_max,shortwave_radiation_sum",
    ...
    #[test]
    fn daily_and_hourly_temperature_gaps_are_nullable_not_zero() {
        let mut raw: Raw = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        // Use future day labels so every row stays in the forecast array.
        raw.daily.time = vec![i64::MAX - 10, i64::MAX - 9, i64::MAX - 8, i64::MAX - 7];
        raw.daily.temperature_2m_max = vec![None, Some(0.0), Some(f64::NAN)];
        raw.daily.temperature_2m_min = vec![Some(75.0), None, Some(f64::INFINITY)];
        raw.hourly.time = vec![1, 3601, 7201, 10801];
        raw.hourly.temperature_2m = vec![None, Some(0.0), Some(f64::NEG_INFINITY)];
        let snapshot = raw.into_snapshot();
        assert_eq!(snapshot.daily[0].temp_max_f, None);
        assert_eq!(snapshot.daily[0].temp_min_f, Some(75.0));
        assert_eq!(snapshot.daily[1].temp_max_f, Some(0.0));
        for day in &snapshot.daily[2..] {
            assert_eq!(day.temp_max_f, None);
            assert_eq!(day.temp_min_f, None);
        }
        assert_eq!(
            snapshot.hourly.iter().map(|h| h.temp_f).collect::<Vec<_>>(),
            vec![None, Some(0.0), None, None]
        );

        // Nulls and completely absent arrays must parse, not reject the
        // complete forecast (rain/wind evidence remains usable).
        let mut json: serde_json::Value = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        json["daily"]["temperature_2m_max"] = serde_json::json!([null, 0.0]);
        json["daily"]
            .as_object_mut()
            .unwrap()
            .remove("temperature_2m_min");
        json["hourly"]
            .as_object_mut()
            .unwrap()
            .remove("temperature_2m");
        let raw: Raw = serde_json::from_value(json).unwrap();
        assert_eq!(raw.daily.temperature_2m_max, vec![None, Some(0.0)]);
        assert!(raw.daily.temperature_2m_min.is_empty());
        assert!(raw.hourly.temperature_2m.is_empty());
    }

    #[test]
    fn wind_and_humidity_missing_invalid_and_zero_are_distinct() {
        let mut raw: Raw = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        let start = Utc::now().timestamp() + 86400;
        raw.daily.time = (0..6).map(|i| start + i * 86400).collect();
        raw.hourly.time = raw.daily.time.clone();
        raw.daily.wind_speed_10m_max = vec![
            None,
            Some(0.0),
            Some(-1.0),
            Some(f64::NAN),
        ...
        raw.hourly.wind_speed_10m = raw.daily.wind_speed_10m_max.clone();
        raw.hourly.relative_humidity_2m =
            vec![None, Some(0.0), Some(-1.0), Some(101.0), Some(f64::NAN)];
        let snapshot = raw.into_snapshot();
        let expected_wind = vec![None, Some(0.0), None, None, None, None];
        assert_eq!(
            snapshot
                .daily
                .iter()
                .map(|d| d.wind_max_mph)
                .collect::<Vec<_>>(),
            expected_wind
        );
        assert_eq!(
        ...
        assert_eq!(
            snapshot
                .hourly
                .iter()
                .map(|h| h.humidity_pct)
                .collect::<Vec<_>>(),
            vec![None, Some(0), None, None, None, None]
        );

        let mut json: serde_json::Value = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        json["daily"]["wind_speed_10m_max"] = serde_json::json!([null, 0]);
        json["hourly"]["relative_humidity_2m"] = serde_json::json!([null, 0]);
        json["hourly"]
            .as_object_mut()
            .unwrap()
            .remove("wind_speed_10m");
        let raw: Raw = serde_json::from_value(json).unwrap();
        assert_eq!(raw.daily.wind_speed_10m_max, vec![None, Some(0.0)]);
        assert_eq!(raw.hourly.relative_humidity_2m, vec![None, Some(0.0)]);
        assert!(raw.hourly.wind_speed_10m.is_empty());
    }
    ...
    #[test]
    fn rain_null_short_and_invalid_arrays_do_not_publish_dry_weather() {
        let mut raw: Raw = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        raw.daily.time = vec![i64::MAX - 10, i64::MAX - 9, i64::MAX - 8, i64::MAX - 7];
        raw.daily.precipitation_sum = vec![None, Some(0.0), Some(-1.0)];
        raw.hourly.time = vec![1, 3601, 7201, 10801];
        raw.hourly.precipitation = vec![None, Some(0.0), Some(f64::NAN)];
        let snapshot = raw.into_snapshot();
        assert_eq!(
            snapshot
                .daily
        ...
        let mut json: serde_json::Value = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        json["daily"]["precipitation_sum"] = serde_json::json!([null, 0.0]);
        json["hourly"]
            .as_object_mut()
            .unwrap()
            .remove("precipitation");
        let raw: Raw = serde_json::from_value(json).unwrap();
        assert_eq!(raw.daily.precipitation_sum, vec![None, Some(0.0)]);
        assert!(raw.hourly.precipitation.is_empty());
    }

    #[test]
    fn preceding_hour_rain_is_normalized_without_delaying_the_forecast() {
        let mut raw: Raw = serde_json::from_str(CURRENT_SAMPLE).unwrap();
        raw.hourly.time = vec![0, 3600, 7200];
        raw.hourly.precipitation = vec![Some(10.0), Some(0.4), Some(0.0)];
        raw.hourly.precipitation_probability = vec![Some(100), Some(50), Some(0)];
        raw.hourly.temperature_2m = vec![Some(70.0), Some(80.0), Some(90.0)];
        let snapshot = raw.into_snapshot();
        assert_eq!(snapshot.hourly[0].temp_f, Some(70.0));
        assert_eq!(snapshot.hourly[0].precip_in, Some(0.4));
        assert_eq!(snapshot.hourly[0].precip_probability, Some(50));
        assert_eq!(snapshot.next_n_hours_precip_in(1, 0), Some(0.4));
        assert_eq!(snapshot.next_n_hours_precip_weighted_in(1, 0), Some(0.2));
        assert_eq!(snapshot.hourly[2].precip_in, None);
    }
}
